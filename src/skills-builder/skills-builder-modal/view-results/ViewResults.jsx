...
const ViewResults = () => {
  ...
  const productTypeNames = useRef(useProductTypes());

  useEffect(() => {
    const getAllRecommendations = async () => {
      // eslint-disable-next-line max-len
      const { jobInfo, results } = await getRecommendations(jobSearchIndex, productSearchIndex, careerInterests, productTypeNames.current);
      ...
      sendTrackEvent('edx.skills_builder.recommendation.shown', {
        ...
        selected_recommendations: {
          ...
          courserun_keys: getProductKeys(results[0].recommendations),
        },
        is_default: true,
      });
    };
    ...
    isLoading ? (
      ...
      <Stack gap={4.5} className="pb-4.5">
        ...
        <RecommendationStack
          ...
          productTypeNames={productTypeNames.current}
        />
      </Stack>
    )
  );
